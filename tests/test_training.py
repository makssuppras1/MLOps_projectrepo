...
class TestSetSeed:
    ...
    def test_set_seed_makes_training_deterministic(self):
        ...
        assert torch.allclose(initial_weight_1, initial_weight_2, atol=1e-6), \
...
class TestCollateFn:
    ...
    def test_collate_fn_returns_correct_shapes(self, tokenizer):
        ...
        assert labels.shape == (batch_size,), \
            f"Labels shape incorrect: expected ({batch_size},), got {labels.shape}"
    ...
    def test_collate_fn_pads_to_same_length(self, tokenizer):
        ...
        assert input_ids.shape[0] == input_ids.shape[0], \
    ...
    def test_collate_fn_returns_correct_dtypes(self, tokenizer):
        ...
        assert input_ids.dtype == torch.long, \
            f"Input IDs should be torch.long, got {input_ids.dtype}"
        assert attention_mask.dtype == torch.long, \
            f"Attention mask should be torch.long, got {attention_mask.dtype}"
        assert labels.dtype == torch.long, \
            f"Labels should be torch.long, got {labels.dtype}"
    ...
    def test_collate_fn_respects_max_length(self, tokenizer):
        ...
        max_length = 32
        batch = [
            ("This is a long text that will be truncated if the max length is reached during tokenization", torch.tensor(0)),
        ...
        assert input_ids.shape[1] <= max_length, \
            f"Sequence length {input_ids.shape[1]} exceeds max_length {max_length}"
...
class TestTrainingIntegration:
    ...
    def test_training_step_reduces_loss(self):
        ...
        assert avg_last_steps < avg_first_steps, \
            f"Average loss should decrease over training steps. First 3 steps avg: {avg_first_steps:.6f}, Last 3 steps avg: {avg_last_steps:.6f}. Losses: {[f'{l:.6f}' for l in losses]}"
    ...
    def test_model_parameters_updated_after_training_step(self):
        ...
        assert params_changed, \
            "Model parameters should be updated after training step. No parameters changed."

    def test_multiple_training_steps_continue_reducing_loss(self):
        """Test that loss continues to decrease over multiple training steps."""
        set_seed(42)
        model = MyAwesomeModel()
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-2)
        ...
        assert avg_second_half < avg_first_half, \
            f"Loss should decrease over training steps. First half avg: {avg_first_half}, Second half avg: {avg_second_half}, Losses: {losses}"
