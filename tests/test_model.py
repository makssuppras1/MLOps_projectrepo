...
class TestMyAwesomeModel:
    ...
    def test_model_initialization_default_config(self):
        ...
        assert model.model_name == 'distilbert-base-uncased', f"Expected 'distilbert-base-uncased' model name, got {model.model_name}"
    ...
    def test_model_output_shapes(self):
        ...
        assert output['logits'].shape == (batch_size, num_labels), f"Logits shape mismatch: expected ({batch_size}, {num_labels}), got {output['logits'].shape}"
        assert output['probs'].shape == (batch_size, num_labels), f"Probs shape mismatch: expected ({batch_size}, {num_labels}), got {output['probs'].shape}"
        assert output['preds'].shape == (batch_size,), f"Preds shape mismatch: expected ({batch_size},), got {output['preds'].shape}"
    ...
    def test_model_probabilities_sum_to_one(self):
        ...
        assert torch.allclose(probs_sum, torch.ones(batch_size), atol=1e-6), f"Probabilities do not sum to 1 across class dimension. Got sums: {probs_sum}"
    ...
    def test_model_predictions_in_range(self):
        ...
        assert torch.all(output['preds'] < model.num_labels), f"Predictions exceed num_labels ({model.num_labels}). Max: {output['preds'].max()}"
    ...
    def test_model_freeze_encoder(self):
        ...
        assert trainable_after_freeze < trainable_before, f"Freezing encoder should reduce trainable params. Before: {trainable_before}, After: {trainable_after_freeze}"
    ...
    def test_model_num_trainable_params(self):
        ...
        assert trainable_params == total_params, f"With freeze_encoder=False, all params should be trainable. Trainable: {trainable_params}, Total: {total_params}"
    ...
    def test_model_gradient_flow(self):
        ...
        assert params_without_grads == 0, f"Some trainable parameters did not receive gradients: {params_without_grads} params without grads"
    ...
    def test_model_different_batch_sizes(self, batch_size):
        ...
        assert output['logits'].shape == (batch_size, 5), f"Logits shape incorrect for batch_size={batch_size}: expected ({batch_size}, 5), got {output['logits'].shape}"
        assert output['preds'].shape == (batch_size,), f"Preds shape incorrect for batch_size={batch_size}: expected ({batch_size},), got {output['preds'].shape}"
    ...
    def test_model_different_sequence_lengths(self, seq_length):
        ...
        assert output['logits'].shape == (batch_size, 5), f"Logits shape incorrect for seq_length={seq_length}: expected ({batch_size}, 5), got {output['logits'].shape}"
    ...
    def test_model_eval_mode(self):
        ...
        assert torch.allclose(output1['logits'], output2['logits'], atol=1e-6), "Model outputs differ in eval mode (dropout should be disabled). This indicates dropout is still active."
    ...
    def test_model_no_grad_context(self):
        ...
        assert output['logits'].requires_grad is False, "Output logits should not require grad in torch.no_grad() context" 